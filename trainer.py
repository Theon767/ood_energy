...
class Trainer(object):
    def __init__(self, model, train_in_loader, train_out_loader,
                 val_in_loader, val_out_loader, test_in_loader, test_out_loader, device):
        # dataloaders
        ...
        self.test_in_loader = test_in_loader
        self.test_out_loader = test_out_loader
    ...
    def train(self, epoch):
        ...
        for (x_in, y_in), (x_out, y_out) in zip(self.train_in_loader, self.train_out_loader):
            ...
            x_out = torch.reshape(x_out, (-1, 3, 32, 32))
            y_out = -1 * torch.ones(x_out.size(0), dtype=y_in.dtype)

            x = torch.cat((x_in, x_out), dim=0).to(self.device)
            y = torch.cat((y_in, y_out), dim=0).to(self.device)

            logits = self.model(x)
            nll_loss = F.cross_entropy(logits[:len(y_in)], y[:len(y_in)])
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for x, y in self.test_in_loader:
                x = x.to(self.device)
                ...
                scores.append(energy)
                labels.append(torch.zeros_like(y))

            # OOD dataset
            for x, y in self.test_out_loader:
                x = x.to(self.device)
                ...
                scores.append(energy)
                labels.append(torch.ones_like(y))
